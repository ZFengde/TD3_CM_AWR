...
class Consistency_Model:
    ...
    def action_log_prob(self, state, model, repeats=50): # TODO, check carefully here to see if there is any problem
        # Generate metric for all candidates
        batch_size = state.shape[0]
        state_rpt = th.repeat_interleave(state.unsqueeze(1), repeats=repeats, dim=1)
        scaled_action = self.batch_multi_sample(model=model, state=state_rpt)
        # std = scaled_action.std(dim=1, unbiased=False)  # std for a batch of actions w.r.t. each dimension
        # normalized_actions = scaled_action / (std.unsqueeze(1) + 1e-8)
        distance_matrix = th.cdist(scaled_action, scaled_action, p=2).detach()

        # sample one of the candidates for each batch
        random_indices = th.randint(0, scaled_action.shape[1], (batch_size,))
        sampled_action = scaled_action[th.arange(batch_size), random_indices]
        sampled_distance_matrix = distance_matrix[th.arange(batch_size), random_indices]
        log_prob = (-sampled_distance_matrix.mean(dim=-1) * np.sqrt(self.action_dim)).unsqueeze(1)

        return sampled_action, log_prob
    
    def awr_weight(self, critic, actor, state, action):
